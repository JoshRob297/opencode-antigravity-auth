...
- **Gemini 3.7 Flash Support** - Full native access to `gemini-3.7-flash` (low, medium, high variants). Automatically spoofs the official Antigravity CLI client signature (`antigravity/cli/...`), bypassing the upstream 404 / preview-access restriction.
- **Gemini 3.6 Flash Support** - Added multi-tier backend model resolution (`gemini-3.6-flash-{low,medium,high}`) with dynamic thinking levels.
- **Gemini 3.5 Flash Support** - Multi-tier thinking resolution support for `gemini-3.5-flash-low` and `gemini-3-flash-agent`.

### Fixed

- **IAM 403 / `cloudaicompanion.instances.completeTask` Fix** - Fixed `403 PERMISSION_DENIED` on default project `rising-fact-p41fc` by correcting `CODE_ASSIST_METADATA.platform` from `MACOS`/`WINDOWS` to `"PLATFORM_UNSPECIFIED"` and updating the discovery User-Agent to `"antigravity"`. Now correctly discovers and persists the account's real `managedProjectId`.
- **Auto-Update Version Comparison** - Fixed semver check in auto-update checker hook to prevent false-positive downgrade attempts.

### Changed

- **Open Source Community Revival** - Maintained and revived from the archived upstream `NoeFabris/opencode-antigravity-auth` project.
- **Automated CI Overhaul** - Streamlined GitHub Actions to a clean, fast Node.js CI test & build pipeline with 1,006 passing tests.

### Removed

- **Dead Publishing Workflows** - Removed obsolete npm release workflows, unused Pi runner scripts, and defunct funding links.
...
- **Quota check and account management in auth login** - Added new `--quota` and `--manage` options to the `auth login` command for checking account quota status and managing accounts directly from the CLI ([#284](https://github.com/JoshRob297/opencode-antigravity-auth/issues/284))
...
- **Prevent toast spam for rate limit warnings** - Added 5-second debounce for rate limit warning toasts to prevent notification flooding when multiple requests hit rate limits simultaneously ([#286](https://github.com/JoshRob297/opencode-antigravity-auth/issues/286))
...
- **Robust handling for capacity/5xx errors** - Implemented comprehensive retry logic for model capacity and server errors, achieving parity with Antigravity-Manager's behavior
  ...
  - Added capacity retry limit to prevent infinite loops ([#263](https://github.com/JoshRob297/opencode-antigravity-auth/issues/263))
...
- **Auth headers aligned with official Gemini CLI** - Updated authentication headers to match the official Antigravity/Gemini CLI behavior, reducing "account ineligible" errors and potential bans ([#178](https://github.com/JoshRob297/opencode-antigravity-auth/issues/178))
...
- **`quiet_mode` now properly suppresses all toast notifications** - Fixed `quiet_mode: true` in `antigravity.json` not suppressing "Status dialog dismissed" and other toast notifications ([#207](https://github.com/JoshRob297/opencode-antigravity-auth/issues/207))
...
See [releases](https://github.com/JoshRob297/opencode-antigravity-auth/releases) for previous versions.
